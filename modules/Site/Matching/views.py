...
def matching(request,x,y,size):
    ...
    if request.method=='POST': #search particular block, display it, may display patches around it
            ...
            for pos in positions:
                if pos['org'][0] == x and pos['org'][1]==y:
                    ...
                    if g[0] == size:
                         ...
                         out_flevels = out_flevels.paginator.page(pno)
        ...
        for line in f:
            ...
            if isOrg:
                ...
                if len(flevel_in_match)!=0:
                    flevels.append(flevel_in_match)
                score_in_match={}
                file_in_match={}
                pos_in_match={}
                plc_in_match={}
                ...
                regex = re.compile('flevel:\s+(.[0-9.]*)')
                r = regex.search(line)
                flevel_in_match[idx]=r.group(1)
                pos_in_match[idx]=(x,y)
                candpattern = "cand_"+str(size)+"_("+str(org_x)+"_"+str(org_y)+")_("+str(x)+"_"+str(y)+")_("+str(idx)+").png"
                plcpattern="cand_plc_"+str(size)+"_("+str(org_x)+"_"+str(org_y)+")_("+str(x)+"_"+str(y)+")_("+str(idx)+")"+"_("+ str(int(flevel_in_match[idx])+1)+").png"
        ...
        out_flevels = getpage(request,flevels,item_per_page)
    ...
    return render(request,'Matching/matching.html',{'form':form,'files':out_files, 'plcs':out_plcs, 'scores':out_scores,'light':out_light,'flevels':out_flevels,'positions':out_pos,'jump':jumpto})
