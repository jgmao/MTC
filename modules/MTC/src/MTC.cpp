        ...
        */
        ...
        if (qNode.size().height==DEBUG_SIZE&&qNode.offset().x==DEBUG_X && qNode.offset().y==DEBUG_Y)
          ...
          cand2.DisplayAll(dummy,1,2);*/
				...
				//UpdatePBSet(qNode,matchCandid[index]);//modify it to store the lighting corrected candidate
...
#     endif
    ...
    for (int i=0; i< (int)matchCandid.size(); i++)
      ...
      candidExt.ExtendBoundary(qNode.overlap(),0);//20130605
        ...
        Tensor<T,cn> candTPSSBefore = lighting::ComputeTPSS(candid, 0.01);
       ...
       // candid.GetExtendTensor(1,1,1,1).Display();
        Tensor<T,cn> candTPSS = lighting::ComputeTPSS(candid, 0.01);
...
#   ifdef RECORD_EVERYTHING
    ...
    {
          everything<<"candid "<<j<<" with LU: ("<<matchCandid[j].x+qNode.overlap().height<<","<<matchCandid[j].y+qNode.overlap().width<<"), light: "<<ldist[j]<<" , score: "<<vdist[j]<<endl;
    }
    everything.close();
#   endif
    int accepted = -1;
