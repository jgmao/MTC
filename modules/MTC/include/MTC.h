...
private:
	...
	int IsAcceptPredict(const vector<Point3i>& matchCandid,  const vector<double>& matchScore, QTree<T,cn>& qNode,MetricModifier metricModifier, int level=-1);//! 20130913 level -1 means no start with 0 foot , no LC
