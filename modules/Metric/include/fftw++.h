...
#include <FComplex>
typedef std::FComplex<double> FComplex;
#endif
...
namespace Array {
static const array1<FComplex> NULL1;
static const array2<FComplex> NULL2;
static const array3<FComplex> NULL3;
}
...
inline FComplex *FComplexAlign(size_t size)
{
  FComplex *v;
  Array::newAlign(v,size,sizeof(FComplex));
  return v;
}
...
// Obsolete names:
#define FFTWFComplex FComplexAlign
...
protected:
  ...
  unsigned int realsize(unsigned int n, FComplex *in, FComplex *out=NULL) {
    return (!out || in == out) ? 2*(n/2+1) : n;
  }
  
  unsigned int realsize(unsigned int n, FComplex *in, double *out) {
    return realsize(n,in,(FComplex *) out);
  }
  
  unsigned int realsize(unsigned int n, double *in, FComplex *out) {
    return realsize(n,(FComplex *) in,out);
  }
...
public:
  ...
  static void Shift(FComplex *data, unsigned int nx, unsigned int ny,
                    int sign=0) {
    const unsigned int nyp=ny/2+1;
    FComplex *pstop=data+nx*nyp;
    if(nx % 2 == 0) {
      int pinc=2*nyp;
      for(FComplex *p=data+nyp; p < pstop; p += pinc) {
      ...
      if(sign) {
        ...
        for(unsigned int i=0; i < nx; i++) {
          double iarg=i*arg;
          FComplex zeta(cos(iarg),sign*sin(iarg));
          FComplex *datai=data+i*nyp;
  ...
  static void Shift(FComplex *data, unsigned int nx, unsigned int ny,
    ...
    if(nx % 2 == 0 && ny % 2 == 0) {
      ...
      if(sign) {
        ...
        for(unsigned i=0; i < nx; i++) {
          double iarg=i*argx;
          FComplex zetax(cos(iarg),sign*sin(iarg));
          FComplex *datai=data+nyzp*i;
          for(unsigned j=0; j < ny; j++) {
            double jarg=j*argy;
            FComplex zeta=zetax*FComplex(cos(jarg),sign*sin(jarg));
            FComplex *dataij=datai+nzp*j;
  ...
  virtual fftw_plan Plan(FComplex *in, FComplex *out)=0;
  
  inline void CheckAlign(FComplex *p, const char *s) {
    if((size_t) p % sizeof(FComplex) == 0) return;
    std::cerr << "WARNING: " << s << " array is not " << sizeof(FComplex)
  ...
  void Setup(FComplex *in, FComplex *out=NULL) {
    if(!Wise) LoadWisdom();
    bool alloc=!in;
    if(alloc) in=FComplexAlign((doubles+1)/2);
  ...
  void Setup(FComplex *in, double *out) {Setup(in,(FComplex *) out);}
  void Setup(double *in, FComplex *out=NULL) {Setup((FComplex *) in,out);}
  ...
  virtual void Execute(FComplex *in, FComplex *out, bool=false) {
    fftw_execute_dft(plan,(fftw_complex *) in,(fftw_complex *) out);
  }
    
  FComplex *Setout(FComplex *in, FComplex *out) {
#ifndef NO_CHECK_ALIGN    
    CheckAlign(in,"input");
  ...
  void fft(FComplex *in, FComplex *out=NULL) {
    out=Setout(in,out);
    Execute(in,out);
  }
    
  void fft(double *in, FComplex *out=NULL) {
    fft((FComplex *) in,out);
  }
  
  void fft(FComplex *in, double *out) {
    fft(in,(FComplex *) out);
  }
  
  void fft0(FComplex *in, FComplex *out=NULL) {
    out=Setout(in,out);
    Execute(in,out,true);
  }
    
  void fft0(double *in, FComplex *out=NULL) {
    fft0((FComplex *) in,out);
  }
  
  void fft0(FComplex *in, double *out) {
    fft0(in,(FComplex *) out);
  }
  
  void Normalize(FComplex *out) {
  ...
  virtual void fftNormalized(FComplex *in, FComplex *out=NULL) {
    out=Setout(in,out);
    Execute(in,out);
    Normalize(out);
  }
  
  void fftNormalized(FComplex *in, double *out) {
    out=(double *) Setout(in,(FComplex *) out);
    Execute(in,(FComplex *) out);
    Normalize(out);
  }
  
  void fftNormalized(double *in, FComplex *out) {
    fftNormalized((FComplex *) in,out);
  }
  
  void fft0Normalized(FComplex *in, FComplex *out=NULL) {
    out=Setout(in,out);
    Execute(in,out,true);
    Normalize(out);
  }
  
  void fft0Normalized(FComplex *in, double *out) {
    out=(double *) Setout(in,(FComplex *) out);
    Execute(in,(FComplex *) out,true);
    Normalize(out);
  }
  
  void fft0Normalized(double *in, FComplex *out) {
    fft0Normalized((FComplex *) in,out);
  }
  
  void fftNormalized(FComplex *in, FComplex *out,
...
// Compute the FComplex Fourier transform of n FComplex values.
// Before calling fft(), the arrays in and out (which may coincide) must be
// allocated as FComplex[n].
//
// Out-of-place usage: 
//
...
class fft1d : public fftw {
  unsigned int nx;
public:  
  fft1d(unsigned int nx, int sign, FComplex *in=NULL, FComplex *out=NULL)
    : fftw(2*nx,sign), nx(nx) {Setup(in,out);} 
  
#ifdef __Array_h__
  fft1d(int sign, const Array::array1<FComplex>& in,
        const Array::array1<FComplex>& out=Array::NULL1)
    : fftw(2*in.Nx(),sign), nx(in.Nx()) {Setup(in,out);} 
#endif  
  
  fftw_plan Plan(FComplex *in, FComplex *out) {
    return fftw_plan_dft_1d(nx,(fftw_complex *) in,(fftw_complex *) out,
                            sign,effort);
  }
};
  
// Compute the FComplex Fourier transform of M FComplex vectors, each of
// length n.
// Before calling fft(), the arrays in and out (which may coincide) must be
// allocated as FComplex[M*n].
//
// Out-of-place usage: 
//
...
//
// Notes:
//   stride is the spacing between the elements of each FComplex vector;
...
public:  
  mfft1d(unsigned int nx, int sign, unsigned int M=1, unsigned int stride=1,
         unsigned int dist=0, FComplex *in=NULL, FComplex *out=NULL)
  ...
  void fftNormalized(FComplex *in, FComplex *out=NULL) {
    fftw::fftNormalized(in,out,nx,M,stride,dist);
  }
};
  
// Compute the FComplex Fourier transform of n real values, using phase sign -1.
// Before calling fft(), the array in must be allocated as double[n] and
// the array out must be allocated as FComplex[n/2+1]. The arrays in and out
// may coincide, allocated as FComplex[n/2+1].
//
// Out-of-place usage: 
//
...
// 
// Notes:
//   in contains the n real values stored as a FComplex array;
//   out contains the first n/2+1 FComplex Fourier values.
//
class rcfft1d : public fftw {
  unsigned int nx;
public:  
  rcfft1d(unsigned int nx, FComplex *out=NULL)
    : fftw(2*(nx/2+1),-1,nx), nx(nx) {Setup(out);}
  
  rcfft1d(unsigned int nx, double *in, FComplex *out=NULL)
    : fftw(realsize(nx,in,out),-1,nx), nx(nx) {Setup(in,out);}
  
#ifdef __Array_h__
  rcfft1d(unsigned int nx, const Array::array1<FComplex>& out)
  ...
  void Execute(FComplex *in, FComplex *out, bool=false) {
    fftw_execute_dft_r2c(plan,(double *) in,(fftw_complex *) out);
  }
};
  
// Compute the real inverse Fourier transform of the n/2+1 FComplex values
// corresponding to the non-negative part of the frequency spectrum, using
// phase sign +1.
// Before calling fft(), the array in must be allocated as FComplex[n/2+1]
...
// 
// Notes:
//   in contains the first n/2+1 FComplex Fourier values.
//   out contains the n real values stored as a FComplex array;
//
...
public:  
  ...
  crfft1d(unsigned int nx, FComplex *in, double *out=NULL)
    : fftw(realsize(nx,in,out),1,nx), nx(nx) {Setup(in,out);} 
  ...
  crfft1d(unsigned int nx, const Array::array1<FComplex>& in)
...
// FComplex[M*(n/2+1)]. The arrays in and out may coincide,
// allocated as FComplex[M*(n/2+1)].
//
// Out-of-place usage: 
//
...
// Compute the real inverse Fourier transform of M FComplex vectors, each of
...
// allocated as FComplex[M*(n/2+1)] and the array out must be allocated as
...
public:
  mcrfft1d(unsigned int nx, unsigned int M=1, unsigned int stride=1,
           unsigned int dist=0, FComplex *in=NULL, double *out=NULL)
...
// Compute the FComplex two-dimensional Fourier transform of nx times ny
// FComplex values. Before calling fft(), the arrays in and out (which may
// coincide) must be allocated as FComplex[nx*ny].
//
// Out-of-place usage: 
//
...
//
// Note:
//   in[ny*i+j] contains the ny FComplex values for each i=0,...,nx-1.
//
class fft2d : public fftw {
  unsigned int nx;
  unsigned int ny;
public:  
  fft2d(unsigned int nx, unsigned int ny, int sign, FComplex *in=NULL,
        FComplex *out=NULL)
    : fftw(2*nx*ny,sign), nx(nx), ny(ny) {Setup(in,out);} 
  
#ifdef __Array_h__
  fft2d(int sign, const Array::array2<FComplex>& in,
        const Array::array2<FComplex>& out=Array::NULL2)
...
// Compute the FComplex two-dimensional Fourier transform of nx times ny real
// values, using phase sign -1.
// Before calling fft(), the array in must be allocated as double[nx*ny] and
// the array out must be allocated as FComplex[nx*(ny/2+1)]. The arrays in
// and out may coincide, allocated as FComplex[nx*(ny/2+1)].
//
// Out-of-place usage: 
//
...
// 
// Notes:
//   in contains the nx*ny real values stored as a FComplex array;
//   out contains the upper-half portion (ky >= 0) of the FComplex transform.
//
class rcfft2d : public fftw {
  unsigned int nx;
  unsigned int ny;
public:  
  rcfft2d(unsigned int nx, unsigned int ny, FComplex *out=NULL)
    : fftw(2*nx*(ny/2+1),-1,nx*ny), nx(nx), ny(ny) {Setup(out);} 
  
  rcfft2d(unsigned int nx, unsigned int ny, double *in, FComplex *out=NULL)
    : fftw(nx*realsize(ny,in,out),-1,nx*ny), nx(nx), ny(ny) {Setup(in,out);} 
  
#ifdef __Array_h__
  rcfft2d(unsigned int ny, const Array::array2<FComplex>& out)
  ...
  void Execute(FComplex *in, FComplex *out, bool shift=false) {
...
// nx*(ny/2+1) FComplex values corresponding to the spectral values in the
// half-plane ky >= 0, using phase sign +1.
// Before calling fft(), the array in must be allocated as
// FComplex[nx*(ny+1)/2] and the array out must be allocated as
// double[nx*ny]. The arrays in and out may coincide,
// allocated as FComplex[nx*(ny/2+1)].
//
// Out-of-place usage (input destroyed):
//
...
// 
// Notes:
//   in contains the upper-half portion (ky >= 0) of the FComplex transform;
//   out contains the nx*ny real values stored as a FComplex array.
//
class crfft2d : public fftw {
  unsigned int nx;
  unsigned int ny;
public:  
  crfft2d(unsigned int nx, unsigned int ny, FComplex *in=NULL) :
    fftw(2*nx*(ny/2+1),1,nx*ny), nx(nx), ny(ny) {Setup(in);} 
  
  crfft2d(unsigned int nx, unsigned int ny, FComplex *in, double *out) :
    fftw(nx*realsize(ny,in,out),1,nx*ny), nx(nx), ny(ny) {Setup(in,out);} 
  ...
  crfft2d(unsigned int ny, const Array::array2<FComplex>& in)
...
// Compute the FComplex three-dimensional Fourier transform of
// nx times ny times nz FComplex values. Before calling fft(), the arrays in
// and out (which may coincide) must be allocated as FComplex[nx*ny*nz].
//
// Out-of-place usage: 
//
...
//
// Note:
//   in[nz*(ny*i+j)+k] contains the (i,j,k)th FComplex value,
...
public:  
  fft3d(unsigned int nx, unsigned int ny, unsigned int nz,
        int sign, FComplex *in=NULL, FComplex *out=NULL)
    : fftw(2*nx*ny*nz,sign), nx(nx), ny(ny), nz(nz) {Setup(in,out);} 
  
#ifdef __Array_h__
  fft3d(int sign, const Array::array3<FComplex>& in,
        const Array::array3<FComplex>& out=Array::NULL3)
...
// Compute the FComplex two-dimensional Fourier transform of
// nx times ny times nz real values, using phase sign -1.
// Before calling fft(), the array in must be allocated as double[nx*ny*nz]
// and the array out must be allocated as FComplex[nx*ny*(nz/2+1)]. The
// arrays in and out may coincide, allocated as FComplex[nx*ny*(nz/2+1)].
//
// Out-of-place usage: 
//
...
// 
// Notes:
//   in contains the nx*ny*nz real values stored as a FComplex array;
//   out contains the upper-half portion (kz >= 0) of the FComplex transform.
//
class rcfft3d : public fftw {
  ...
  unsigned int nz;
public:  
  rcfft3d(unsigned int nx, unsigned int ny, unsigned int nz, FComplex *out=NULL)
  ...
  rcfft3d(unsigned int nx, unsigned int ny, unsigned int nz, double *in,
          FComplex *out=NULL) : fftw(nx*ny*realsize(nz,in,out),-1,nx*ny*nz),
                               nx(nx), ny(ny), nz(nz) {Setup(in,out);} 
  
#ifdef __Array_h__
  rcfft3d(unsigned int nz, const Array::array3<FComplex>& out)
...
// Compute the real two-dimensional inverse Fourier transform of the
// nx*ny*(nz/2+1) FComplex values corresponding to the spectral values in the
// half-plane kz >= 0, using phase sign +1.
// Before calling fft(), the array in must be allocated as
// FComplex[nx*ny*(nz+1)/2] and the array out must be allocated as
// double[nx*ny*nz]. The arrays in and out may coincide,
// allocated as FComplex[nx*ny*(nz/2+1)].
//
// Out-of-place usage (input destroyed):
//
...
// 
// Notes:
//   in contains the upper-half portion (kz >= 0) of the FComplex transform;
//   out contains the nx*ny*nz real values stored as a FComplex array.
//
class crfft3d : public fftw {
  ...
  unsigned int nz;
public:  
  crfft3d(unsigned int nx, unsigned int ny, unsigned int nz, FComplex *in=NULL)
    : fftw(2*nx*ny*(nz/2+1),1,nx*ny*nz), nx(nx), ny(ny), nz(nz)
  {Setup(in);} 
  
  crfft3d(unsigned int nx, unsigned int ny, unsigned int nz, FComplex *in,
          double *out=NULL) : 
  ...
  crfft3d(unsigned int nz, const Array::array3<FComplex>& in)
