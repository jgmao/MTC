...
*/
                       ...
                       MetricModifier stsim2_modifer, bool debug)
  {
    ...
    if (boundary_cut==FilterBoundary::FILTER_BOUND_EXTEND)
    {
      int rr = A.size().height/2;
      cv::copyMakeBorder(tsA,A,rr,rr,rr,rr,cv::BORDER_REFLECT,0);
      cv::copyMakeBorder(tsB,B,rr,rr,rr,rr,cv::BORDER_REFLECT,0);
    }
    filterA.updateData(A);
    filterA.decompose();
    vector<Tensor<double,2>>& pyrA = filterA.getSpaceDomainPyr();

    filterB.updateData(B);
    filterB.decompose();
    vector<Tensor<double,2>>& pyrB = filterB.getSpaceDomainPyr();


    int nDir = filterA.getDir();
    int nLevel = filterA.getLevel();
    //pyrA[0].size().Print();
    //pyrB[0].size().Print();
    vector<Tensor<double,2>> mu_A(pyrA.size());
    vector<Tensor<double,2>> mu_B(pyrB.size());
...
*/
  ...
  double Compare(const Mat& tsA, const Mat& tsB, CompareCriteria criteria,
                 FilterBank& filterA, FilterBank& filterB, const Size3& subWinSize,const Size3& subWinStep,
                 Printable_t param1, Printable_t param2, Printable_t param3, Printable_t param4, bool debug)
  {
    /*va_list ap;
  ...
  */
    ...
    */
        ...
        if (modifier==(int)MetricModifier::STSIM3_LSE) //gj01142013
        {
            //cv::error(CV_StsNotImplemented,"not done");
            return -1;
           // return ComputeSTSIM3_LSE(tsA,tsB,tempSubSize,tempSubStep,nLevel,nDir,downsample,FilterBoundary(bd_cut),debug);
          }
        else
          return ComputeSTSIM2(tsA,tsB,filterA, filterB, tempSubSize,tempSubStep,downsample, FilterBoundary(bd_cut),FeaturePoolType(pool_type), MetricModifier(modifier),debug);
        //else
        //  return T1.ComputeSSIM(T2,subWinSize,subWinStep,(int)param1,(int)param2, FILTER_BOUND_VALID, param4, param5);
      }
    else if (criteria == CompareCriteria::COMPARE_CRITERIA_SUBJECTIVE)
