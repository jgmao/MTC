...
vector<Steerable::data_type>& Steerable::buildSCFpyr(Steerable::c_data_ref im, int nLevel, int nDir, int twidth, bool subsample)
{
	...
	pyr_freq.push_back(imdft.FreqComplexFilter(toComplex(hi0mask)));
	pyr_space.clear();
...
//	mylib::DisplayMat(hi1mask);
  ...
  //mylib::DisplayMat(lo1mask,"lowmask");
	...
	buildSCFpyrLevs(loDft.FreqComplexFilter(lo1mask),rst,nLevel-1,nDir,next_twidth,subsample);
...
cv::Mat Steerable::DFT(cv::Mat& A)
{
	...
	if (A.channels() == 1 || A.channels() == 3)
		cv::dft(toComplex( convertTo(A, CV_64F + ((A.channels()-1)<<CV_CN_SHIFT))),rst);
	else
		cv::dft( convertTo(A,CV_64F+ ((A.channels()-1)<<CV_CN_SHIFT)),rst);
	return rst;
}
